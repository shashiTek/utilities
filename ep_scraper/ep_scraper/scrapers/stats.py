"""
ep_scraper/scrapers/stats.py
-----------------------------
StatsScraper
  Reads the `teams` collection to build a player lookup, groups players
  by (league_slug, season_slug), scrapes the EP league stats page
  (the only page with server-side rendered stat tables), matches rows
  back to players by their /player/ href, and upserts into `stats`.
...
class StatsScraper(BaseScraper):
            ...
            with self.http:
                for combo_num, ((league_slug, season_slug), group_meta) in enumerate(
                    ...
                    upserted = self._scrape_combo(
                        combo_num, len(groups),
                        league_slug, season_slug,
                        group_meta, player_lookup, bio_lookup,
                    )
    ...
    def _build_player_lookup(self) -> dict[str, dict]:
        """
        Build: normalised_player_url → { player_name, team_name, team_doc_id,
        ...
        for team_doc in self._teams_repo.find({"athlete": {"$exists": True}}):
            ...
            for athlete in athletes:
                ...
                at       = athlete.get("@type", "")
                position = (
                    "G" if "Goalkeeper" in at or "Goaltender" in at else "F/D"
                )
    ...
    def _scrape_combo(
        ...
        if already_done == len(group_urls):
            self.log.info(
                ...
                combo_num, total_combos, len(group_urls), league_slug, season_slug,
            )
            return 0
        ...
        self.log.info(
            ...
            league_slug.upper(), season_slug,
            len(group_urls), kinds,
        )
    ...
    def _scrape_kind(
        ...
        base = (
            f"{self.cfg.ep_base_url}/league/{league_slug}"
            f"/stats/{season_slug}"
            f"{'?tab=goalies' if kind == 'goalie' else ''}{'&' if kind == 'goalie' else '?'}page="
        )
